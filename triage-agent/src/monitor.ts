...
function buildKafkaConfig(): KafkaConfig {
  ...
  const config: KafkaConfig = { clientId: "triage-monitor", brokers };
...
function verdictBadge(verdict: string): string {
  if (verdict === "TRUE_POSITIVE")  return `${BOLD}${BG_RED}  TRUE POSITIVE  ${RESET}`;
  if (verdict === "FALSE_POSITIVE") return `${BOLD}${BG_GREEN}  FALSE POSITIVE  ${RESET}`;
...
function formatVerdict(v: TriageVerdict): string {
  ...
  lines.push(`${BOLD}${WHITE}[${ts}] ${v.category}${RESET}  ${verdictBadge(v.verdict)}`);
  ...
  if (v.evidenceItems?.length) {
    ...
    for (const e of v.evidenceItems) {
      const icon  = e.supportsVerdict ? `${GREEN}[+]${RESET}` : `${RED}[-]${RESET}`;
...
async function main() {
  ...
  console.log("╔══════════════════════════════════════════════════════════════════════╗");
  console.log("║          O365 Triage Agent — Verdict Monitor                        ║");
  console.log(`╚══════════════════════════════════════════════════════════════════════╝${RESET}`);
  console.log(`${DIM}Connecting to Kafka broker: ${process.env.KAFKA_BROKERS ?? "192.168.0.103:9092"}${RESET}`);
